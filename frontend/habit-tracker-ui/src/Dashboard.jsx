...
export default function Dashboard({ auth, onLogout }) {
  ...
  useEffect(() => { loadHabits(); /* eslint-disable-next-line */ }, [loadHabits]);
  useEffect(() => { loadWeeklyTotal(); loadStreaks(); /* eslint-disable-next-line */ }, [loadWeeklyTotal, loadStreaks, habits.length]);
  ...
  async function deleteHabit(habit_id) {
    ...
    const res = await fetch(`${API}/api/habits/${habit_id}?user_id=${user.user_id}`, { method: "DELETE" });
    const data = await res.json().catch(()=>({error:"Failed"}));
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            ...
            <div className="text-2xl font-semibold text-slate-900">{weeklyTotal} min</div>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            <div className="text-sm text-slate-500">Current streak</div>
            <div className="text-2xl font-semibold text-slate-900">{currentStreak} day{currentStreak===1?"":"s"}</div>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            <div className="text-sm text-slate-500">Longest streak</div>
            <div className="text-2xl font-semibold text-slate-900">{longestStreak} day{longestStreak===1?"":"s"}</div>
          </div>
        </section>

        {/* Add habit */}
        <section>
          <form onSubmit={addHabit} className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
            <input
        ...
        <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          ...
          {loading ? (
            ...
            habits.map(h => (
              <div key={h.habit_id} className="grid grid-cols-12 px-4 py-2 border-b last:border-b-0 border-slate-100 text-sm items-center">
                <div className="col-span-4 sm:col-span-4">
                  {editingId === h.habit_id ? (
                    <input className="w-full rounded-xl border border-slate-300 px-2 py-1" value={editName} onChange={(e)=>setEditName(e.target.value)} />
                  ) : h.habit_name}
                </div>
                <div className="col-span-2 sm:col-span-2">
                  {editingId === h.habit_id ? (
                    <input className="w-full rounded-xl border border-slate-300 px-2 py-1" value={editDuration} onChange={(e)=>setEditDuration(e.target.value)} />
                  ) : h.duration}
                </div>
                <div className="col-span-3 sm:col-span-3">{h.timestamp?.replace('T',' ').slice(0,16)}</div>
                <div className="hidden sm:block sm:col-span-2">{h.duration_minutes ?? "—"}</div>
                <div className="col-span-3 sm:col-span-1 flex justify-end gap-2">
                  {editingId === h.habit_id ? (
                    <>
                      <button className="text-xs rounded-lg px-2 py-1 bg-emerald-600 text-white" onClick={()=>saveEdit(h.habit_id)}>Save</button>
                      <button className="text-xs rounded-lg px-2 py-1 bg-slate-200" onClick={()=>setEditingId(null)}>Cancel</button>
                    </>
                  ) : (
                    <>
                      <button className="text-xs rounded-lg px-2 py-1 bg-slate-200" onClick={()=>startEdit(h)}>Edit</button>
                      <button className="text-xs rounded-lg px-2 py-1 bg-red-600 text-white" onClick={()=>deleteHabit(h.habit_id)}>Delete</button>
                    </>
                  )}
                </div>
              </div>
